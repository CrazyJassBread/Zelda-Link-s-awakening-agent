...
class ZeldaEnv(gym.Env):
    def __init__(self, rom_path):
        ...
        # 定义动作空间
        self.action_space = spaces.Discrete(8)  # 8个动作
    ...
    def _get_state(self):
        ...
        position = self.pyboy.memory[0xDBAE]
        equipment = self.pyboy.memory[0xDB00] * 256 + self.pyboy.memory[0xDB01]
        inventory = [self.pyboy.memory[0xDB02 + i] for i in range(10)]
        keys = self.pyboy.memory[0xDBD0]
        arrows = self.pyboy.memory[0xDB45]
        bombs = self.pyboy.memory[0xDB4D]
        magic_powder = self.pyboy.memory[0xDB4C]
        secret_shells = self.pyboy.memory[0xDB0F]
        golden_leaves = self.pyboy.memory[0xDB15]
        instruments = [self.pyboy.memory[0xDB65 + i] for i in range(8)]
        map_status = [self.pyboy.memory[0xD800 + i] for i in range(256)]
...
# 使用环境
env = ZeldaEnv(
    rom_path="D:\codes\codes_pycharm\da_chuang\Legend_of_Zelda\Legend_of_Zelda.gb"
)
