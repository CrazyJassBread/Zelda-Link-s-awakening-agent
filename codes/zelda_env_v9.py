...
import time
import mediapy as media  # 添加mediapy库用于视频录制
from skimage.transform import downscale_local_mean  # 添加skimage库用于降低分辨率
...
######################################################################################################
   ...
   # 在代码开头添加
import os
os.environ["FFMPEG_BINARY"] = r"C:\Users\Leslie Clover\AppData\Local\Microsoft\WinGet\Packages\Gyan.FFmpeg_Microsoft.Winget.Source_8wekyb3d8bbwe\ffmpeg-7.1.1-full_build\bin\ffmpeg.exe"  # 替换为你的实际路径
...
class ZeldaEnv(gym.Env):
    ...
    def reset(self, seed=None, options={}):
        ...
        # 如果启用视频录制并且不是第一次重置，关闭之前的视频写入器
        if self.save_video and self.reset_count > 0:
    ...
    def get_game_state_reward(self, action=None):
        ...
        health_stable_reward = 0  # 生命值稳定奖励，稳定一定步数给予奖励，藉以代替杀掉敌人的情况
        ...
        if position_tuple not in self.visited_positions:
            exploration_reward = 20  # 访问新位置给予奖励
    ...
    def render(self, reduce_res=True):
        """渲染游戏画面，可选择是否降低分辨率"""
        game_pixels_render = self.pyboy.screen.ndarray[:, :, 0:1]  # 获取游戏屏幕的红色通道
        if reduce_res:
            game_pixels_render = (
                downscale_local_mean(game_pixels_render, (2, 2, 1))  # 降低分辨率
            ).astype(np.uint8)
    ...
    def start_video(self):
        ...
        # 创建视频保存的路径
        base_dir = self.s_path / Path("video_recordings")
        base_dir.mkdir(exist_ok=True)
        
        # 创建不同分辨率的视频文件
        full_name = Path(f"zelda_full_reset_{self.reset_count}").with_suffix(".mp4")
        model_name = Path(f"zelda_reduced_reset_{self.reset_count}").with_suffix(".mp4")
        map_name = Path(f"zelda_map_reset_{self.reset_count}").with_suffix(".mp4")
        
        # 初始化视频写入器
        self.full_frame_writer = media.VideoWriter(
            base_dir / full_name, (144, 160), fps=30, input_format="gray"
        )
        self.full_frame_writer.__enter__()
        
        self.model_frame_writer = media.VideoWriter(
            base_dir / model_name, (72, 80), fps=30, input_format="gray"
        )
        self.model_frame_writer.__enter__()
        
        # 地图视图
        explore_map_size = 48  # 探索地图的大小
        self.map_frame_writer = media.VideoWriter(
            base_dir / map_name, (explore_map_size, explore_map_size), 
            fps=30, input_format="gray"
        )
        self.map_frame_writer.__enter__()
        
        print(f"Started video recording for reset {self.reset_count}")

    def add_video_frame(self):
        """添加当前帧到视频文件"""
        # 添加完整分辨率帧
        full_frame = self.render(reduce_res=False)
        self.full_frame_writer.add_image(full_frame[:,:,0])
        
        # 添加降低分辨率的帧
        reduced_frame = self.render(reduce_res=True)
        self.model_frame_writer.add_image(reduced_frame[:,:,0])
        
        # 为地图视图创建一个简单的可视化
        # 获取角色当前位置
        sprite = self.pyboy.get_sprite(2)
        current_x, current_y = sprite.x, sprite.y
        
        # 创建一个表示探索区域的地图
        explore_map_size = 48
        explore_map = np.zeros((explore_map_size, explore_map_size), dtype=np.uint8)
        
        # 标记已访问的位置
        for pos in self.visited_positions:
            x, y = pos
            # 将位置映射到地图上
            map_x = int(x / 160 * explore_map_size)
            map_y = int(y / 144 * explore_map_size)
            if 0 <= map_x < explore_map_size and 0 <= map_y < explore_map_size:
                explore_map[map_y, map_x] = 128  # 灰色表示已访问
        
        # 标记当前位置
        map_x = int(current_x / 160 * explore_map_size)
        map_y = int(current_y / 144 * explore_map_size)
        if 0 <= map_x < explore_map_size and 0 <= map_y < explore_map_size:
            explore_map[map_y, map_x] = 255  # 白色表示当前位置
            
        # 标记目标位置
        target_x = int(self.target_x / 160 * explore_map_size)
        target_y = int(self.target_y / 144 * explore_map_size)
        if 0 <= target_x < explore_map_size and 0 <= target_y < explore_map_size:
            explore_map[target_y, target_x] = 200  # 浅灰色表示目标
            
        self.map_frame_writer.add_image(explore_map)

    def step(self, action):
        # 如果启用了视频保存功能，并且是第一步，启动视频记录
        ...
        # 在环境结束时关闭视频写入器
        if self.save_video and (terminated or truncated):
            if self.full_frame_writer is not None:
                self.full_frame_writer.close()
