...
# ####################################################################################################
#from v8 添加战斗的跟随，一开始先主要进行杀敌人的操作，然后再继续靠近钥匙，这个问题还没解决，先看一下代码     #
######################################################################################################
"""
TODO:
想要完成的是如何解决前期只注重于杀死敌人，然后再靠近钥匙，因为根据结果来看会学习到直接靠近钥匙，
这样动作就直接是移动按键，来不及杀死敌人，
那么方法
1. 设置奖励函数，将自己在一定步骤下健康值保持不变的情况下下进行奖励，代替敌人被杀，度过战斗期。
2. 设置比例，在最后相加的时候，移动的奖励和战斗的奖励的比例进行修改。
...
class ZeldaEnv(gym.Env):
    def __init__(self, rom_path, config=None):
        ...
        self.target_x = 38  # 钥匙的x坐标
        self.target_y = 43  # 钥匙的y坐标
        ...
        self.last_health = 1
        self.enemy_phase = True  # 是否处于敌人阶段
        self.last_enemy_count = 2  # 初始假设有2个敌人

         # 添加记录获取钥匙时间的列表和文件路径
        self.key_obtained_times = []
        self.results_file = Path(self.s_path) / "key_obtained_results_v10.txt"
    ...
    def get_game_state_reward(self, action=None):
        ...
        # 打印敌人状态
        print(f"当前敌人数量: {current_enemy_count}, 敌人阶段: {'是' if self.enemy_phase else '否'}")
    ...
    def step(self, action):
        ...
        # 检查是否有敌人在附近
        enemy_nearby = self.is_enemy_nearby(threshold=40)
        
        # 根据敌人状态调整动作策略
        if not self.enemy_phase and (self.valid_actions[action] in [WindowEvent.PRESS_BUTTON_A, WindowEvent.PRESS_BUTTON_B]):
            # 如果敌人已清除但仍在使用攻击键，随机选择移动方向
            action = np.random.randint(0, 4)  # 0-3对应方向键
            print(f"探索阶段不允许攻击: 将动作从{original_action}(A/B键)改为{action}(方向键)")
        elif self.enemy_phase:
            # 在敌人阶段
            if enemy_nearby:
                # 敌人在附近时，强制使用攻击键
                attack_action = np.random.choice([4, 5])  # 4和5对应A和B键
                if self.valid_actions[action] not in [WindowEvent.PRESS_BUTTON_A, WindowEvent.PRESS_BUTTON_B]:
                    print(f"敌人靠近! 将动作从{action}(方向键)改为{attack_action}(A/B键)")
                    action = attack_action
            elif np.random.random() < 0.4:  # 降低到40%的概率
                # 敌人不在附近时，降低使用攻击键的概率
                if self.valid_actions[action] not in [WindowEvent.PRESS_BUTTON_A, WindowEvent.PRESS_BUTTON_B]:
                    # 使用较低概率改为攻击键
                    attack_action = np.random.choice([4, 5])  # 4和5对应A和B键
                    print(f"敌人阶段但不在附近: 将动作从{action}(方向键)改为{attack_action}(A/B键)")
                    action = attack_action
        ...
        info = {
            ...
            "enemy_phase": self.enemy_phase,
            "enemy_nearby": self.is_enemy_nearby()
        }
    ...
    def get_enemy_positions(self):
        """获取当前场景中所有敌人的位置， 虽然敌人由两块组成，但是位置得计算应该是可以的，但是还是要找敌人即on screen()的情况"""
        enemy_positions = []
        try:
            for idx in range(13, 23):  
                sprite = self.pyboy.get_sprite(idx)
    ...
    def is_enemy_nearby(self, threshold=30):
        """检查是否有敌人在角色附近"""
        # 获取角色位置
        player_sprite = self.pyboy.get_sprite(2)  # 假设角色是精灵2
        player_x, player_y = player_sprite.x, player_sprite.y
