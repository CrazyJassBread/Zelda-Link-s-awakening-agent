...
import time
# #########################
# 向左走 + reward, 向右走 - reward, 保持大体方向, 加了时间
##########################
class ZeldaEnv(gym.Env):
    def __init__(self, rom_path, config=None):
        ...
        self.results_file = Path(self.s_path) / "key_obtained_results_v6.txt"
    ...
    def step(self, action):
        ...
        if self.valid_actions[action] == WindowEvent.PRESS_ARROW_LEFT:
            additional_reward = 20  # 向左走加10点奖励
        elif self.valid_actions[action] == WindowEvent.PRESS_ARROW_RIGHT:
            additional_reward = -20  # 向右走减10点奖励
...
# 训练模型
model.learn(total_timesteps=500000)  # 您可以根据需要调整 timesteps
